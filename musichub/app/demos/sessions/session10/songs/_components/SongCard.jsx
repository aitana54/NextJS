...
export default function SongCard({ title, artist, youtubeId, onToggleLike }) {
  ...
  function handleLike() {
    const next = !liked;
    setLiked(next);
    // Ejemplo de mal uso del estado (no hacerlo así)
    // Usar el valor anterior directamente puede causar errores
    // setCount(count + 1);
    // setCount(count + 1);
    // setCount(count + 1);
    // Forma correcta de actualizar el estado basado en el anterior
  ...
  return (
    <article className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden transition transform hover:scale-105 hover:shadow-md">
      ...
      <div className="p-3 space-y-2">
        <div>
          {/* <p className="text-xs text-gray-900">Contador: {count}</p> */}
